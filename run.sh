...
if [ "$AIRFLOW" = "1" ]; then airflow db init; sleep 10; airflow users create --role Admin --username sporule --email sporule --firstname sporule --lastname sporule --password sporule; (airflow webserver -p 8080 -D &) ;sleep 5;(airflow scheduler -D &); fi
